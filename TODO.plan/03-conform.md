...
| emf | `Emf.parse` → `Metafile` exposing `ok?` and `errors`. **`ok?` is not sufficient on its own**, and truncation is the sharpest case: of the 164 truncations of `valid.emf` that parse without raising, **16 report `ok? == true`** (measured 2026-09-08, same sweep as the allowlist figures below). `after_eof.emf` shows it without truncation too — bytes past the EOF record, measuring `ok? == true, errors == [], records == 16`, the same numbers as the intact file. (Re-measured 2026-09-08. The earlier wording cited a removed-EOF file at `records == 15`; no fixture on disk reproduces that today, and both `short_eof` fixtures measure `ok? == false` with one error. The conclusion is unchanged, the example is now one that reproduces.) Require an EOF record on top of `ok?` (D23). **Do not naively compare the declared record count to the parsed count** — the intact fixture reports `header.n_records == 17` against 16 parsed records, so that offset means something not yet understood. Either work out what it counts and then cross-check, or restrict the pre-pass to EOF presence and trailing-byte detection, both of which were measured catching real corruption. Map `errors` (message, offset, record_code) to `severity: "error"`, `code: "EMF_PARSE"`, `location: {byte_offset: offset}` — `byte_length` stays nil unless the pre-pass computes it. On severity generally: png_conform emits a SYMBOL (`:error`, measured), `Models::Issue::SEVERITIES` is `%w[error warning info]`, and "map as-is" is safe because lutaml coerces — `Issue.new(severity: :error)` stores the String `"error"` and round-trips, while `:nonsense` is still refused with `Lutaml::Model::ValidationError`. The enum guards either way; the coercion is what makes the symbol harmless, and it is worth knowing rather than rediscovering. Truncation raises **`IOError`** at some cut points and `Emf::FormatError` at others — both belong on the conform allowlist, or a corrupt file exits 4 instead of 1. Swept 2026-09-08 across all **364** cut points of `valid.emf`: `IOError` 96, `Emf::FormatError` 88, `EOFError` 16, no exception at 164. **`EOFError` is a subclass of `IOError`**, so rescuing that pair covers every truncation — verified by rescuing exactly the pair and finding nothing escapes. **But the pair is NOT the whole allowlist**: `bad_comment.emf`, already on disk, raises **`NoMethodError`** from inside the gem, which the pair does not catch. **Do not add it to the allowlist**: it is a gem defect, not a corrupt-input signal. In `emf-0.1.0/lib/emf/emr/parser.rb`, `Emr::Parser#extract_emf_plus` guards on the DECLARED `cb_data >= 4` and then ORs together `data.getbyte(0)`, `data.getbyte(1)`, `data.getbyte(2)` and `data.getbyte(3)` without checking the actual bytesize, so each returns nil and the shifts raise. It is called from `Emr::Parser#call`, OUTSIDE the `rescue StandardError` in `#read_record` that would otherwise have reported the same fault as a `ParseError`. Cited by method name rather than by line number, deliberately: this is another gem's source, so any line reference here rots on its next release. Reachable from ordinary corruption, not just this fixture: 40,000 mutated documents produced 4 `NoMethodError` hits, every one in that function. Report it upstream; do not widen the rescue to swallow it. And note the pre-pass is still needed once it is fixed — with a bytesize guard applied, `bad_comment.emf` comes back `ok? == true, errors == []`, because the gem has no way to say "a comment declares more data than it holds" |
