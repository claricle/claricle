...
RSpec.describe Claricle::Cli::Runner do
  ...
  describe "formats" do
    ...
    # pass if the command printed nothing at all. emf, pdf and png are
    # the formats that claim conform; nothing claims convert yet.
    ...
    it "emits a fixed row shape under --json" do
      conform = { "emf" => true, "eps" => false, "pdf" => true, "png" => true, "ps" => false, "svg" => false }
  ...
  # pdf and png both implement conformance_report now; eps and ps never
  # will (D22), so they carry the exit-3 UnsupportedFormat story on. Exit
  # 0 and 1 arrive end to end through both handlers.
  describe "conform" do
