...
RSpec.describe "Claricle metafile handler" do
  ...
  # emf 0.1.0 reads anything above 88 as the extension records, so it
  # raises EOFError on both -- on the declared prefix AND on the full
  ...
  describe "EMF+" do
    ...
    it "carries a structurally valid EMF+ stream, checked without the parser" do
      ...
      # The outer cbData is asserted too: measured, corrupting both of them
      # to 32/16 left every example in this file passing.
  ...
  describe "the EMF+ probe's framing rules" do
    ...
    # The declared cbData is what counts, not the record's padded size.
    # Both carriers are padded, so summing sizes would give 44 + 28.
    it "counts declared payload rather than padded record size" do
