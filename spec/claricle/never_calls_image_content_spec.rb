...
# bytes in `@content`: `#content` slurps the whole file into memory. Only
# path-born is in scope -- `with_path` already writes content-born bytes to
# a temp file, so reaching `#content` there is legitimate.
#
# Samples are driven off the registry: a new inspect-capable format cannot
# skip this file. Where the detector accepts a file the handler then fails
# on, add that failing file too -- a handler that slurps only on the
# failure path would otherwise pass on good input alone.
RSpec.describe "Handlers inspect path-born samples without calling Image#content" do
  ...
  samples = {
    ...
    # zeroed_wrapper.eps: DOS EPS wrapper signature, invalid PostScript
    # range -- refused at the header scan, never reaches the delegate.
